...
const Chatbot = () => {
    ...
    return (
      <div className={`chat-message ${msg.role}`}>
        <div className="message">
          <strong style={{ color: msg.role === 'user' ? '#007bff' : '#6a11cb' }}>
            {roleText}:
          </strong> 
  ...
  return (
    <>
      {isOpen && (
        <div id="chatbot" className={`chatbot ${isOpen ? 'open' : ''}`}>
          <div className="chat-header">
            <span>Chatbot</span>
            <span className="close-btn" onClick={toggleChatbot}>×</span>
          </div>
      ...
      <div id="chatButton" className={`chat-button ${isOpen ? 'hidden' : ''}`} onClick={toggleChatbot}>💬</div>
    </>
  );
};
